...
# Simulated allele frequencies for site-level analysis (as opposed to gene-level analysis) using average mutation rate from Kong et al. 2012
sims_AverU<-read.table("sims_currAF_Eur_varU_averageRate.tab.txt", header=FALSE)
...
sims_AverUExons<-read.table("sims_currAF_Eur_VarU_averageRate_forExons.txt", header=FALSE)
...
sims_fourTypes<-read.table("sims_currAF_Eur_varU.tab", header=TRUE)
...
sim1M<-read.table("1M_eurDistrib.txt",header=FALSE)
...
simIG<-read.table("eurDistrib_nophase2_CORRECTED.txt",header=FALSE)

# Simulations by gene (equivalent to the sum of deleterious allele frequency per gene)
eurbygene<-read.table("sims_euroByGene.tab",header=TRUE)
...
setEPS()
postscript("Fig1.eps",width=7.5,height=7)
...
abline(v=((1-((log10(mean(sims_AverULOG[1:100000]))+5)*-1))*5)+6,col="red",lwd = 2) #SIM
abline(v=((1-((log10(FIN20K)+5)*-1))*5)+6,col="green",lwd = 2) #FIN
abline(v=((1-((log10(INF)+3)*-1))*5)+16,col="blue",lwd = 2) #INF
...
setEPS()
postscript("Fig2.eps",width=6.8,height=6.35)
...
plot(log10(mutationRates),foldIncrease,pch=19,xlab="Log10(u)",ylab="Fold increase")#,main="B") 
labels=c("\nnonCpGtv","nonCpGti","CpGtv","")
text(log10(mutationRates),foldIncrease, labels=labels, cex= 1, pos=4,col=c("black","black","black","white"))
labels=c("","","","CpGti\n")
text(log10(mutationRates),foldIncrease, labels=labels, cex= 1, pos=2,col=c("white","white","white","black"))

mtext("A", side = 1, adj=0, line=-42, cex=1.3, font=2)
mtext("B", side = 1, adj=0, line=-9, cex=1.3, font=2)
...
text(seq(1, 27, by=1), par("usr")[3] - 0.2, labels = c("","","","","","","","","","","DHCR7","PEX7","PRF1","ASS1","SMARCAL1","NPC1","GBE1","GALC","TK2","STAR","CLN5","TPP1","GAA","SLC22A5","POMGNT1","GAN","HEXA"), srt = 45, pos = 1, xpd = TRUE,cex = 0.5)
text(seq(1, 27, by=1), par("usr")[3] - 0.2, labels = c("CFTR","IDUA","SMPD1","POLG","FAH","LAMB3","ERCC8","ASPA","HSD17B4","PPT1","","","","","","","","","","","","","","","","",""), srt = 45, pos = 1, xpd = TRUE,cex = 0.5,font=2)
...
plot(counsyl$freqEur_Counsyl,counsyl$EuroExAC,xlim = c(0,max(counsyl$freqEur_Counsyl)),ylim=c(0,max(counsyl$freqEur_Counsyl)),xlab="Allele frequency in Counsyl",ylab="Allele frequency in ExAC",pch=16)
...
x<-mean(sims_AverUExons$V1[1:100000])
size=function(q){((q/aver_u)^2)/(2*pi)}
abline(v=size(x),col="red")
title(outer=F,adj=0,main="A",cex=10,col="black",font=4,line=1)
HconstSize<-hist(constSize$X0,plot=F)
Hsims_AverU<-hist(sims_AverU$V1[1:100000],plot=F,breaks = HconstSize$breaks)
...
plot(Hsims_AverU,ylab='Log10(Density)',xlab='Allele frequency',main="",col="grey",yaxt = "n")
...
plot(HconstSize,ylab='Log10(Density)',xlab='Allele frequency',main="",col="grey",yaxt = "n")
...
sims_AverULOG=replace(sims_AverU$V1,sims_AverU$V1==0,0.5e-6)
...
abline(v=log10(mean(sims_AverU$V1)),col="red",lwd = 2)
axis(2,labels=c(0,1,10,100),at=c(0,0.33,0.66,1))
...
plot(0:1,0:1,type="n",xlim=c(0.5,8.5),ylim=c(-7,-2),axes=FALSE,ann=FALSE)
vioplot(log10(Tobs),log10(Aobs),log10(Bobs),log10(Cobs),log10(Dobs),log10(Eobs),log10(Fobs),log10(eobs),drawRect=FALSE,col=c("grey","grey","grey","grey","grey","grey","grey","grey","white"),add=TRUE)
axis(side=1,at=1:8,labels=c("A","B","C","D","E","F","G","ExAC"))
...
rect(4.6,-7,5.4,Dzero,col="grey")
rect(5.6,-7,6.4,Ezero,col="grey")
rect(6.6,-7,7.4,Fzero,col="grey")
rect(7.6,-7,8.4,ezero,col="white")
...
points(x=5,y=log10(mean(sim10MPOI$V1)), pch="-", cex=2,col="red")
points(x=6,y=log10(mean(simIGPOI$V1)), pch="-", cex=2,col="red")
points(x=7,y=log10(mean(sims_LargerAverUPOI$V1)), pch="-", cex=2,col="red")
points(x=8,y=log10(mean(ExAC$eur_freq)), pch="-", cex=2,col="red")
...
x<-as.numeric(x)
x<-x*100
text(7,-6.65,paste(x,"%",sep=""),cex=1)
x<-format(round(((nrow(ExAC)-(length(eobs)))/nrow(ExAC)), 2), nsmall = 2)
x<-as.numeric(x)
x<-x*100
text(8,-6.73,paste(x,"%",sep=""),cex=1)
...
p_ascB = function (n_ind){
  prob_asc = 1-((1-1.014918e-05)*(1+1.014918e-05-(1.014918e-05*1/16)))^n_ind
  return(prob_asc)
}
p_ascC = function (inbreed_coef){
  prob_asc = 1-((1-1.014918e-05)*(1+1.014918e-05-(1.014918e-05*inbreed_coef)))^32881
  return(prob_asc)
}
...
par(mfrow=c(1,3))
plot.function(p_ascA,min(sims_AverUExons$V1),max(sims_AverUExons$V1),xlab="Allele frequency",ylab="P")
plot.function(p_ascB,100,5000000,xlab="Sample size",ylab="P")
plot.function(p_ascC,0,1/2,xlab="Inbreeding coeficient",ylab="P")
dev.off()
